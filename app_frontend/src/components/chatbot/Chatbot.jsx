      ...
      {isChatOpen && (
        ...
        `}>
          ...
          <div className={`p-6 flex justify-between items-center relative overflow-hidden ${isDarkMode ? 'bg-slate-900/50' : 'bg-blue-600 text-white'}`}>
            ...
            <button onClick={() => setIsChatOpen(false)} className="p-2 rounded-xl bg-white/5 hover:bg-white/10 transition-all">
              <X className="w-5 h-5" />
            </button>
          </div>
          ...
          <div className={`flex-1 overflow-y-auto p-6 space-y-6 flex flex-col custom-scrollbar ${isDarkMode ? 'bg-black/20' : 'bg-slate-50'}`}>
              ...
              <div key={idx} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'} animate-in fade-in slide-in-from-bottom-2`}>
                <div className={`
                  ...
                  ${msg.role === 'user' 
                    ...
                    : `${isDarkMode ? 'bg-white/5 border border-white/10 text-slate-200' : 'bg-white border border-gray-100 text-gray-800'} rounded-bl-none`}
                `}>
            ...
            {isChatLoading && (
              <div className="flex justify-start">
                <div className={`rounded-[24px] rounded-bl-none px-5 py-4 flex items-center gap-1 ${isDarkMode ? 'bg-white/5 border border-white/10' : 'bg-white border text-gray-400'}`}>
          ...
          {/* Input */}
          <form onSubmit={handleSendMessage} className={`p-4 pb-8 flex flex-col gap-3 border-t ${isDarkMode ? 'bg-slate-900 border-white/10' : 'bg-white border-gray-100'}`}>
