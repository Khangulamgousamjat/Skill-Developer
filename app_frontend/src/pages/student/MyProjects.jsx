            ...
            {personalProjects.map(project => (
              <div key={project.id} className="bg-[var(--color-surface)] border border-[var(--color-border)] p-5 rounded-xl group relative overflow-hidden">
                <div className="absolute top-0 right-0 p-3 opacity-0 group-hover:opacity-100 transition-opacity flex gap-2">
                  <button onClick={() => handleOpenModal(project)} className="p-1.5 bg-white/10 backdrop-blur-md rounded-lg text-blue-400 hover:text-blue-300">
                    <Edit2 size={14} />
                  </button>
                  <button onClick={() => handleDelete(project.id)} className="p-1.5 bg-white/10 backdrop-blur-md rounded-lg text-red-400 hover:text-red-300">
                    <Trash2 size={14} />
                  </button>
                </div>
