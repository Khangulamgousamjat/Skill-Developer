...
import { 
  ...
  Video as VideoIcon, Clock, Calendar, 
  User, Bookmark, Share2,
  PlayCircle, Sparkles, TrendingUp, X
} from 'lucide-react';
...
export default function StudentVideosPage() {
  ...
  const fetchVideos = async () => {
    try {
      ...
      toast.error('Failed to load learning vault');
      console.error(err);
    } finally {
      setLoading(false);
    }
  ...
  const categories = ['All', 'Technical', 'Soft Skills', 'Architecture', 'Design'];

  const filteredVideos = videos.filter(v => {
    const matchesSearch = v.title.toLowerCase().includes(searchQuery.toLowerCase()) || 
                          (v.teacher_name && v.teacher_name.toLowerCase().includes(searchQuery.toLowerCase()));
      ...
      <div className="space-y-8 pb-12 animate-in fade-in duration-700">
        ...
        <div className="relative rounded-[40px] overflow-hidden bg-gradient-to-br from-indigo-600 via-purple-600 to-pink-500 p-8 md:p-12 text-white shadow-2xl">
          ...
          <div className="relative z-10 max-w-2xl px-2">
            <div className="flex items-center gap-2 mb-4 bg-white/10 backdrop-blur-md w-fit px-4 py-1.5 rounded-full border border-white/20">
              ...
              <span className="text-[10px] font-black uppercase tracking-widest leading-none">Premium Learning Modules</span>
            </div>
            ...
            <div className="flex flex-wrap gap-4">
              <div className="flex items-center gap-2 bg-black/20 backdrop-blur-md px-5 py-2.5 rounded-2xl border border-white/10">
                <VideoIcon size={18} className="text-indigo-200" />
        ...
        {loading ? (
          ...
          <div className="py-24 text-center bg-[var(--color-surface)] border border-dashed border-[var(--color-border)] rounded-[40px] flex flex-col items-center justify-center">
            <PlayCircle size={64} className="mx-auto mb-6 text-[var(--color-border)] opacity-30" />
            <h3 className="text-xl font-bold text-[var(--color-text-primary)]">No modules found</h3>
            <p className="text-[var(--color-text-muted)] mt-2 italic px-8">Try a different search or category.</p>
          </div>
        ) : (
            ...
            <AnimatePresence>
              {filteredVideos.map((video) => (
                ...
                >
                  <div className="aspect-video bg-black relative overflow-hidden">
                    {video.thumbnail_url ? (
                      ...
                      <div className="w-full h-full flex items-center justify-center bg-gradient-to-tr from-indigo-900 to-purple-900">
                        <VideoIcon size={48} className="text-white/10" />
                      </div>
                    )}
                    <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                       <div className="w-16 h-16 rounded-full bg-white/10 backdrop-blur-xl border border-white/20 flex items-center justify-center scale-75 group-hover:scale-100 transition-transform">
                          <Play fill="white" className="text-white shadow-xl" size={24} />
                       </div>
                    </div>
                    ...
                    {video.duration ? (
                      <div className="absolute bottom-4 right-4 px-2.5 py-1 bg-black/60 backdrop-blur-md text-white text-[10px] font-bold rounded-lg border border-white/10 font-mono">
                  ...
                  <div className="p-6">
                    <h3 className="font-bold text-[var(--color-text-primary)] font-sora text-lg mb-2 line-clamp-1 group-hover:text-[var(--color-primary)] transition-colors leading-tight">{video.title}</h3>
                    <p className="text-xs text-[var(--color-text-muted)] line-clamp-2 mb-6 h-8 leading-relaxed italic">{video.description}</p>
                    
                    <div className="pt-6 border-t border-[var(--color-border)] flex items-center justify-between">
                       <div className="flex items-center gap-3">
                          <div className="w-9 h-9 rounded-xl bg-[var(--color-surface-2)] border border-[var(--color-border)] flex items-center justify-center overflow-hidden shadow-inner">
                             {video.teacher_photo ? (
                          ...
                          <div>
                             <p className="text-[9px] font-black uppercase tracking-widest text-[var(--color-text-muted)]">Instructor</p>
                             <p className="text-xs font-bold text-[var(--color-text-primary)]">{video.teacher_name || 'Protocol Teacher'}</p>
                          </div>
                       </div>
                       <div className="flex items-center gap-1 text-[var(--color-text-muted)]">
                          <button className="p-2 hover:bg-[var(--color-surface-2)] rounded-xl hover:text-[var(--color-primary)] transition-all"><Bookmark size={18} /></button>
                          <button className="p-2 hover:bg-[var(--color-surface-2)] rounded-xl hover:text-[var(--color-primary)] transition-all"><Share2 size={18} /></button>
                       </div>
                    </div>
                  </div>
                </motion.div>
        ...
        <AnimatePresence>
          {selectedVideo && (
            <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4 bg-black/95 backdrop-blur-xl transition-all">
              <motion.div 
              ...
              >
                <button 
                  ...
                  className="absolute top-6 right-6 z-10 p-3 rounded-full bg-black/20 backdrop-blur-md text-white hover:bg-rose-500 transition-colors shadow-lg"
                >
                  <X size={24} />
                </button>

                <div className="aspect-video bg-black flex items-center justify-center group relative shadow-inner">
                  <video 
                    autoPlay
                    controls 
                ...
                <div className="p-8 md:p-12 text-left">
                   <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                      <div>
                         <h2 className="text-3xl font-bold font-sora text-[var(--color-text-primary)] mb-4">{selectedVideo.title}</h2>
                         <div className="flex flex-wrap items-center gap-6 text-[10px] font-black uppercase tracking-[0.2em] text-[var(--color-text-muted)]">
                            <span className="flex items-center gap-2 bg-[var(--color-surface-2)] px-3 py-2 rounded-xl border border-[var(--color-border)]">
                                <Calendar size={14} className="text-[var(--color-primary)]" /> 
                                {new Date(video.created_at).toLocaleDateString()}
                            </span>
                            <span className="flex items-center gap-2 bg-[var(--color-primary)]/5 text-[var(--color-primary)] px-3 py-2 rounded-xl border border-[var(--color-primary)]/10 font-bold">
                                <Sparkles size={14} /> {selectedVideo.category}
                            </span>
                         </div>
                      </div>
                      <div className="flex items-center gap-4 py-4 px-6 bg-[var(--color-surface-2)] border border-[var(--color-border)] rounded-[32px] shadow-sm">
                         <div className="w-12 h-12 rounded-2xl overflow-hidden border-2 border-[var(--color-primary)]/20 shadow-lg">
                         ...
                         <div>
                            <p className="text-[10px] font-black uppercase tracking-widest text-[var(--color-text-muted)]">Expert Curator</p>
                            <p className="text-sm font-bold text-[var(--color-text-primary)]">{selectedVideo.teacher_name || 'Protocol Trainer'}</p>
                         </div>
                      </div>
                   </div>
                   <div className="mt-8 pt-8 border-t border-[var(--color-border)]">
                      <p className="text-[var(--color-text-secondary)] leading-relaxed text-sm md:text-lg max-w-4xl italic">
